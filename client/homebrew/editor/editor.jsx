...
const Editor = createReactClass({
	...
	handleInject : function(injectText){
		...
		if(selection && injectText.includes('\n')) {
			// For block-level snippets (multiline), try to wrap the selection
			// Find the placeholder text between opening {{ and closing }}
			const wrapMatch = injectText.match(/^(\n?{{[^\n]*\n)([\s\S]*?)(\n\s*}}\n?)$/);
			if(wrapMatch) {
				const wrappedText = `${wrapMatch[1]}${selection}\n${wrapMatch[3]}`;
				cm.replaceSelection(wrappedText, 'around');
				cm.focus();
				return;
			}
		}
	...
	jumpToText : function(text, pageNum) {
		if(!this.isText() || !text || !this.codeEditor.current?.codeMirror) return;

		const cm = this.codeEditor.current.codeMirror;
		const fullText = this.props.brew.text;
		const lines = fullText.split('\n');

		// Determine the line range for the target page
		const pageRegex = this.props.renderer == 'V3' ? PAGEBREAK_REGEX_V3 : /\\page/;
		let pageStart = 0;
		let pageEnd = lines.length;
		if(pageNum) {
			let currentPage = 1;
			for (let i = 0; i < lines.length; i++) {
				if(i > 0 && pageRegex.test(lines[i])) currentPage++;
				if(currentPage === pageNum && pageStart === 0) pageStart = i;
				if(currentPage > pageNum) { pageEnd = i; break; }
			}
		}

		// Clean the search text: collapse whitespace, take first chunk
		const searchStr = text.replace(/\s+/g, ' ').slice(0, 80);

		// Search within the page's line range for best match
		let bestLine = -1;
		let bestScore = 0;

		for (let i = pageStart; i < pageEnd; i++) {
			// Build a multi-line window for context matching
			const window = lines.slice(i, Math.min(i + 3, pageEnd)).join(' ').replace(/\s+/g, ' ');
			// Check if the search text appears in this window
			if(window.includes(searchStr)) {
				bestLine = i;
				bestScore = searchStr.length;
				break;
			}
			// Try progressively shorter matches
			for (let len = Math.min(searchStr.length, 40); len >= 8; len -= 4) {
				const partial = searchStr.slice(0, len);
				if(window.includes(partial) && len > bestScore) {
					bestLine = i;
					bestScore = len;
				}
			}
		...
		const targetY = cm.heightAtLine(bestLine, 'local', true);
		cm.scrollTo(null, targetY);
		this.codeEditor.current.setCursorPosition({ line: bestLine, ch: 0 });
